...
class Experimenter:
    def __init__(self,
        ...
        if self.toy:
            ...
            self.lb, self.ub = 0, 1  # Set bounds for toy example
        else:
            assert function is not None, 'have to initialize function if not doing toy experiment'
            self.function = function
            self.lb, self.ub = self.function.bounds.lb, self.function.bounds.ub  # Use function bounds
    ...
    def initialize_surrogate(self):
        ...
        """
        # Generate LHS samples in the unit hypercube and scale to [lb, ub]
        samples = lhs(self.dim_total, self.num_DoE)
        samples = self.lb + (self.ub - self.lb) * samples  # Scale to [lb, ub]
    ...
    def iterate(self):
        ...
        """
        # Step 1: Randomly generate new points in the input space scaled to [lb, ub]
        random_points = self.lb + (self.ub - self.lb) * np.random.rand(self.num_samples, self.dim_total)
        ...
        for epoch in range(self.num_epochs):
            ...
            # print(complements_values)

            # Compute loss for all points
            # print(embeddings_values, complements_values)
